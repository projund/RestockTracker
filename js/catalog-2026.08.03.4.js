(() => {
  ...
  const key=p=>[p.category,p.game||p.brand||p.beyType,p.setName||'',p.productName||p.beyModel].join('|');
  const favorites=new Set(old.filter(p=>p.favorite).map(key));
  const products=[];
  const add=(id,category,data)=>products.push({id,category,retail:0,favorite:favorites.has(key({category,...data})),image:'',imageSource:'',walmart:'',target:'',identifiers:'',notes:'',active:true,...data});
  const tcg={
  ...
  let n=1;
  Object.entries(tcg).forEach(([branch,types])=>{const [game,setName]=branch.split('|');types.forEach(productName=>add(`catalog-tcg-${n++}`,'TCG',{game,setName,productName}))});
  ...
  const wave1=[['Sword Dran 3-60F','Starter'],['Helm Knight 3-80N','Starter'],['Arrow Wizard 4-80B','Starter'],['Scythe Incendio 4-60T','Starter'],['Steel Samurai 4-80T','Booster'],['Horn Rhino 3-80S','Booster'],['Keel Shark 3-60LF','Booster'],['Talon Ptera 3-80B','Booster'],['Knife Shinobi 4-80HN & Keel Shark 3-80F','Dual Pack'],['Chain Incendio 5-60HT & Arrow Wizard 4-60N','Dual Pack'],['Tail Viper 5-80O & Sword Dran 3-60F','Dual Pack'],['Soar Phoenix 9-60GF Deluxe String Launcher Set','Deluxe Set'],['Dranzer Spiral 3-80T Red Edition','Exclusive'],['Xtreme Battle Set: Dagger Dran 4-60R & Tusk Mammoth 3-60T','Stadium Set'],['Transformers Deluxe Set: Optimus Primal 3-60F & Starscream 3-80N','Exclusive'],['Transformers Deluxe Set: Optimus Prime 4-60P & Megatron 4-80B','Exclusive'],['Beystadium','Stadium'],['Winder Launcher','Accessory']];
  ...
  const existingKeys=new Set(products.map(key));
  old.forEach(product=>{if(!existingKeys.has(key(product)))products.push(product)});
